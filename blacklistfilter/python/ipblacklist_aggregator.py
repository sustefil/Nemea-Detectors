...
urfmt = "ipaddr DST_IP,ipaddr SRC_IP,uint64 BYTES,uint64 DST_BLACKLIST,uint64 LINK_BIT_FIELD,uint64 SRC_BLACKLIST," + \
        "time TIME_FIRST,time TIME_LAST,uint32 EVENT_SCALE,uint32 PACKETS,uint16 DST_PORT,uint16 SRC_PORT," + \
        "uint8 DIR_BIT_FIELD,uint8 PROTOCOL,uint8 TCP_FLAGS,uint8 TOS,uint8 TTL"
...
def storeEvent():
    ...
    if UR_Input.SRC_BLACKLIST:
        # SRC address on blacklist
        key = (UR_Input.SRC_IP, UR_Input.PROTOCOL, UR_Input.SRC_BLACKLIST)
    else:
        # We assume that one of SRC_BLACKLIST/DST_BLACKLIST always has non-zero value
        key = (UR_Input.DST_IP, UR_Input.PROTOCOL, UR_Input.DST_BLACKLIST)
    ...
    if key in eventList:
        ...
        if UR_Input.SRC_BLACKLIST:
            event["targets"][str(UR_Input.DST_IP)] = UR_Input.DST_PORT
        else:
            event["targets"][str(UR_Input.SRC_IP)] = UR_Input.SRC_PORT
        ...
        # Insert new event
        if UR_Input.SRC_BLACKLIST:
            # source (of trouble) is the blacklisted address
            source = {str(UR_Input.SRC_IP) : UR_Input.SRC_PORT}
            targets = {str(UR_Input.DST_IP) : UR_Input.DST_PORT}
        else:
            source = {str(UR_Input.DST_IP) : UR_Input.DST_PORT}
            targets = {str(UR_Input.SRC_IP) : UR_Input.SRC_PORT}
        ...
        event = {
            ...
            "byte_count": UR_Input.BYTES,
            "packet_count": UR_Input.PACKETS,
            "event_scale": UR_Input.EVENT_SCALE,
            "src_blacklist": UR_Input.SRC_BLACKLIST,
            "dst_blacklist": UR_Input.DST_BLACKLIST
        }
